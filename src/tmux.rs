//! Thin shell-out wrappers around remote tmux via ssh.
//!
//! Every session lives on the VPS, not the operator's machine. Each `target`
//! parses as `alias[:label]`:
//!
//! - `vps1`        → ssh to `vps1`, tmux session name `helm`
//! - `vps1:deploy` → ssh to `vps1`, tmux session name `helm-deploy`
//!
//! Because each VPS has its own tmux server, session names don't need to
//! include the alias for uniqueness — the alias picks the server (via ssh),
//! the label namespaces sessions within that server.
//!
//! Sessions are created with `tmux new-session -A` (attach-or-create),
//! which is idempotent. `ensure_session` is a single fire-and-forget ssh.
//!
//! Why remote-tmux: persistence survives helm restarts AND network drops
//! AND operator-machine reboots. The session lives on the VPS until the VPS
//! itself reboots or someone runs `tmux kill-server` on it.
//!
//! Text passed to `send_keys` is quoted for POSIX shell evaluation on the
//! remote, so passwords / shell metachars / spaces survive the round-trip.
...
/// Build the remote `ssh <alias> -- <quoted command>` invocation.
fn ssh_cmd(alias: &str, remote: &str) -> Command {
    let mut c = Command::new("ssh");
    c.arg(alias).arg(remote);
    c
}
...
pub fn ensure_session(target: &str) -> Result<()> {
    ...
    let status = ssh_cmd(&alias, &remote)
        .stdout(Stdio::null())
        .stderr(Stdio::inherit())
        .status()
        .context("spawn ssh for tmux new-session")?;
    if !status.success() {
        return Err(anyhow!(
            "remote `tmux new-session` on {alias} failed (exit {})",
...
/// Send a line of text (followed by Enter) to the remote session's active
/// pane. Text is split into two `send-keys` calls — first `-l` (literal,
/// no key-name parsing) for the body, then a bare `Enter`. This avoids
/// every escaping pitfall with characters tmux would otherwise interpret
/// as key names (e.g. text containing the word `Space` or `Enter`).
pub fn send_keys(target: &str, text: &str) -> Result<()> {
    ...
    let status = ssh_cmd(&alias, &remote)
        ...
        .stderr(Stdio::inherit())
        .status()
        .context("spawn ssh for tmux send-keys")?;
    if !status.success() {
        return Err(anyhow!(
            "remote `tmux send-keys` on {alias} failed (exit {})",
            status.code().unwrap_or(-1)
        ));
    }
    Ok(())
...
/// Capture the active pane's contents from the remote session.
pub fn capture(target: &str, lines: u32) -> Result<String> {
    ...
    let out = ssh_cmd(&alias, &remote)
        .output()
        .context("spawn ssh for tmux capture-pane")?;
    if !out.status.success() {
        return Err(anyhow!(
            "remote `tmux capture-pane` on {alias} failed (exit {}): {}",
...
/// user-facing target form for each (e.g. `vps1`, `vps1:deploy`).
pub fn list(alias: &str) -> Result<Vec<String>> {
    let remote = "tmux list-sessions -F '#{session_name}' 2>/dev/null || true";
    let out = ssh_cmd(alias, remote)
        .output()
        .context("spawn ssh for tmux list-sessions")?;
    if !out.status.success() {
        return Err(anyhow!(
            "remote tmux list-sessions on {alias} failed (exit {}): {}",
            out.status.code().unwrap_or(-1),
            String::from_utf8_lossy(&out.stderr).trim()
        ));
    }
...
/// Kill the remote session for `target`.
pub fn kill(target: &str) -> Result<()> {
    ...
    let status = ssh_cmd(&alias, &remote)
        ...
        .stderr(Stdio::inherit())
        .status()
        .context("spawn ssh for tmux kill-session")?;
    if !status.success() {
        return Err(anyhow!(
            "remote `tmux kill-session` on {alias} failed (exit {})",
            status.code().unwrap_or(-1)
        ));
    }
    Ok(())
