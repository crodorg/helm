...
For **in-window panes**, there is no `pane run` yet — panes are local, so the send→read latency is negligible; use `helm pane send` then `helm pane read`.
