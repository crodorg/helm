...
- **`helm shell`** — a tmux session on a chosen host. A real ssh alias (`web`, `vps1`) → the session runs on that device over ssh; this is the ONLY way I drive another machine. By default the session gets a **viewport pane** in the operator's window (`helm pane view`) so they watch live — read-only to *me* (I never type into it), but the operator can type into it (it's a live attach): that's where they enter a doas password or hit Ctrl-C.
...
Then **keep going** — drive the task to its conclusion, pausing only where the discipline below demands it: a real password prompt, a destructive or risky mutation, or a genuinely uncertain pane state. Don't stop to ask "want me to continue?" between routine steps.

**Opt out — "headless" / "in the background":** skip the viewport, run quietly, report back. Only when the operator says so. (This is about *my* visibility, not backgrounding a process — "run the build in the background" is a parked job, not a headless request. When it's ambiguous, the viewport is cheap: open it.)
...
- **Password prompt** (line ending in `:` containing `password`/`passphrase`) → stop, tell the operator, do not type. They type it into the viewport pane (remote) or the drivable pane (local) — a live attach, not read-only to *them*. **Only stop if the password line is the LAST non-empty line.** If a fresh prompt already appears below it, the prompt was satisfied — proceed, don't pester.
...
- **`helm shell`**: by default, open the viewport first — `helm pane view <target>` — so the operator watches from the start (see *the default*); skip it only for headless work. Then `send`/`run`/`read` auto-create the session on first use (1–2s; the first `read` may show an empty pane or the login MOTD — wait one more cycle). To pre-warm the session quietly without a pane: `helm shell open -d <target>`.
